...
const TOOL_NAME = "build_influencer_lead_list";
const ACTOR_ID = "KnmByszcv135yM30G";
...
test("package identity matches the locked naming convention", () => {
  ...
  assert.deepEqual(key, ["mamba-creator-lead-list-all-in-one"]);
  assert.deepEqual(mcp.mcpServers[key[0]].args, ["-y", pkg.name]);
  assert.equal(pkg.name, "@mambalabsdev/mcp-creator-lead-list-all-in-one");
  assert.equal(pkg.mcpName, "com.mambabuilt/mcp-creator-lead-list-all-in-one");
});
